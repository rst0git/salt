...
class SaltTest(testprogram.TestProgramCase):
    ...
    def test_exit_status_unknown_argument(self):
        ...
        # runner.shutdown() should not be unnecessary since the start-up should fail
...
class TestRetcode(object):
    ...
    def test_salt_error(self, salt_cli):
        ...
        ret = salt_cli.run('test.raise_exception', 'salt.exceptions.CommandNotFoundError', minion_tgt='minion')
        assert ret.exitcode == salt.defaults.exitcodes.EX_GENERIC, ret

        ret = salt_cli.run('test.raise_exception', 'salt.exceptions.CommandExecutionError', minion_tgt='minion')
        assert ret.exitcode == salt.defaults.exitcodes.EX_GENERIC, ret

        ret = salt_cli.run('test.raise_exception', 'salt.exceptions.SaltInvocationError', minion_tgt='minion')
        assert ret.exitcode == salt.defaults.exitcodes.EX_GENERIC, ret
        ...
        ret = salt_cli.run('test.echo', '{foo: bar, result: False}', minion_tgt='minion')
        assert ret.exitcode == salt.defaults.exitcodes.EX_GENERIC, ret

        ret = salt_cli.run('test.echo', '{foo: bar, success: False}', minion_tgt='minion')
        assert ret.exitcode == salt.defaults.exitcodes.EX_GENERIC, ret
    ...
    def test_missing_minion(self, salt_cli, salt_master):
        ...
        try:
            # Copy the key
            with salt.utils.files.fopen(good, 'rb') as fhr, salt.utils.files.fopen(bad, 'wb') as fhw:
                fhw.write(fhr.read())
            ret = salt_cli.run('-t', '5', 'test.ping', minion_tgt='minion2')
            assert ret.exitcode == salt.defaults.exitcodes.EX_GENERIC, ret
        finally:
            # Now get rid of it
            try:
