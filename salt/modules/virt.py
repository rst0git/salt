...
def _migrate(dom, dst_uri, **kwargs):
    ...
    if max_downtime:
        try:
            ...
            raise SaltInvocationError("Invalid max_downtime value: %s" % max_downtime)
...
def migrate_non_shared_inc(vm_, target, ssh=False, **kwargs):
    ...
    salt.utils.versions.warn_until(
        ...
        "Use 'migrate' with copy_storage='incremental' instead.",
    )
    return migrate(vm_, target, ssh, copy_storage="incremental", **kwargs)
